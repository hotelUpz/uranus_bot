...
SPECS_UPDATE_INTERVAL_SEC       = 3600.0 # Обновляем список пар раз в час
...
class TradingBot:
                    ...
                    try:
                        ...
                        if getattr(pos, 'is_closed_by_exchange', False):
                            if pos.entry_price > 0.0:
                                ...
                                # Фиксируем в отдельный аудит-лог
                                try:
                                    from CORE._executor import order_logger
                                    order_logger.info(f"[{pos.symbol}] EXIT {pos.side} | Qty: {pos.max_realized_qty} | Price: {exit_pr:.6f} | PnL: {net_pnl:.4f}$ | Type: {current_status}")
                                except: pass
    ...
    async def start(self):
        ...
        if not self.symbol_specs:
            logger.error(f"❌ [HALT] Не удалось получить спецификации символов для '{self.bl_manager.quota_asset}'. Проверьте подключение к API Phemex.")
            await self.stop()
            return
