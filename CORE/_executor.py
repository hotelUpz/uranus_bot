...
order_logger = UnifiedLogger(name="orders")
...
STAKAN_STALE_TIMEOUT_SEC    = 5.0    # Максимальный возраст данных стакана (сек)

# Коэффициент запаса для объема (чтобы точно влезть в лимит при резком движении)
ORDER_QTY_SAFETY_FACTOR     = 0.98


def round_step(value: float, step: float) -> float:
    if not step or step <= 0:
        logger.error(f"❌ Критическая ошибка: round_step получил некорректный шаг {step}. Использую дефолт 1.0")
        step = 1.0
...
class OrderExecutor:
    ...
    async def execute_entry(self, symbol: str, pos_key: str, signal: EntrySignal) -> bool:
        ...
        try:
            ...
            # 1. Получаем стакан и проверяем свежесть
            bids, asks, last_update_ts = self.tb.stakan_stream.get_depth(symbol)
            if not bids or not asks:
                logger.warning(f"[{pos_key}] Стакан не готов (пуст). Пропуск.")
                return False
                
            now = time.time()
            if now - last_update_ts > STAKAN_STALE_TIMEOUT_SEC:
                logger.error(f"🚨 [ЗАЩИТА] [{symbol}] Стакан протух! Последнее обновление {now - last_update_ts:.1f}с назад. Пропуск.")
                return False
            ...
            # 2. Расчет объема с запасом, чтобы точно влезть в маржу даже при резком скачке
            qty = round_step((self.notional_limit * ORDER_QTY_SAFETY_FACTOR) / execution_ref_price, spec.lot_size)
            usdt_vol = qty * execution_ref_price
            
            if qty < spec.min_qty:
                logger.warning(f"[{pos_key}] Qty {qty} меньше минимального {spec.min_qty}. Пропуск.")
                return False

            # --- ЛОГИРОВАНИЕ ДАННЫХ ОРДЕРА ---
            best_price_info = f"Ask1: {signal.init_ask1}" if signal.side == "LONG" else f"Bid1: {signal.init_bid1}"
            log_str = f"🚀 [{symbol}] ENTRY {signal.side} | Qty: {qty} | Vol: {usdt_vol:.2f} USDT | RefPrice: {execution_ref_price} | {best_price_info}"
            logger.info(log_str)
            order_logger.info(log_str)
            # ---------------------------------
                    ...
                    if resp.get("code") == 0:
                        ...
                        async with self.tb._get_lock(pos_key):
                            ...
                            if pos and (pos.current_qty > 0 or getattr(pos, "in_position", False)):
                                ...
                                order_logger.info(f"[{symbol}] ENTRY {signal.side} | Qty: {pos.current_qty} | Price: {execution_ref_price:.6f} | Vol: {entry_usd_vol:.2f} USDT")
        ...
        for idx, order in enumerate(grid_orders):
            ...
            for tp_attempt in range(3):
                try:
                    ...
                    tp_usdt_vol = tp_qty * tp_price
                    
                    log_str = f"🎯 [{symbol}] TP #{idx+1} | {side} | Qty: {tp_qty} | Vol: {tp_usdt_vol:.2f} USDT | Price: {tp_price}"
                    logger.info(log_str)
                    order_logger.info(log_str)

                    resp = await self.client.place_limit_order(
                        symbol, side, tp_qty, tp_price, phemex_pos_side, reduce_only=True
                    )
    ...
    async def execute_exit(self, symbol: str, pos_key: str, order_price: float = 0.0, timeout_sec: float = 0.0) -> bool:
        ...
        try:
            ...
            await self.cancel_all_orders(symbol)
            # Даем микро-паузу, чтобы ядро Phemex 100% успело снять локи с объема
            await asyncio.sleep(WAIT_AFTER_ALL_CALCEL)
