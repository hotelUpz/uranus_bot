...
# ============================================================
# FILE: API/PHEMEX/order.py
# ROLE: Phemex private REST client (Ultra-Fast via curl_cffi)
# ============================================================
import time
import ujson
import hmac
...
class PhemexPrivateClient:
    ...
    # Обрати внимание: aiohttp session больше не используется, но мы оставили аргумент в сигнатуре 
    # для обратной совместимости (чтобы не менять вызовы везде, если не хотим).
    def __init__(self, api_key: str, api_secret: str, session: Any = None, retries: int = 2):
        self.api_key = api_key
        self.secret_bytes = api_secret.encode("utf-8")
        self.retries = retries

        # HTTP/2 + Chrome Impersonation
        ...
        self._lock = asyncio.Lock()
        self._last_send_time = 0
        self.MIN_SEND_INTERVAL = 0.005  # 5мс лимит между вызовами
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                resp = await self.session.request(
                    method, url, data=body_str if body_str else None, headers=headers, timeout=timeout_sec
                )
    ...
    async def place_market_order(self, symbol: str, side: str, qty: float, pos_side: str, reduce_only: bool = False) -> Dict[str, Any]:
        body = {
            "symbol": symbol, "side": side, "orderQtyRq": float_to_str(qty),
            "ordType": "Market", "timeInForce": "ImmediateOrCancel", "posSide": pos_side
        }
        if reduce_only: body["reduceOnly"] = True
        return await self._request("POST", "/g-orders", body=body)
